...
def _split_into_clauses(tokens: list) -> list[tuple[int, int]]:
    """
    Splits a token list into clause *spans* (start_idx, end_idx exclusive)
    on every contrastive conjunction.

    Example
    -------
    tokens = ['delivery', 'fast', 'thi', 'lekin', 'stand', 'toota', 'hua', 'tha']
    returns → [(0, 3), (4, 8)]          # 'lekin' at index 3 is the boundary
    """
    spans   = []
    current_start = 0
...
class POSAspectExtractor:
    ...
    ----------------------------------
    Context windows are now clipped to the clause boundary created by
    contrastive conjunctions (lekin, magar, but …).  This prevents positive
    words from a preceding clause from contaminating the sentiment features
    of an aspect in the subsequent (often negative) clause, and vice-versa.
    """
    ...
    # ------------------------------------------------------------------ #
    #  Aspect discovery (unchanged logic, kept intact)                    #
    # ------------------------------------------------------------------ #

    def discover_aspects(self, dataset: list) -> set:
        """
        Dynamically discovers aspects by using the POS model's conditional
        probability to verify which words are Nouns (NN).
        """
